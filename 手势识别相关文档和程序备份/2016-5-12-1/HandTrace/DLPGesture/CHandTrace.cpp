...
void CHandTrace::on_trackbar(int pos, std::vector<vector<Point>> & contours)
{
	...
	cv::Mat MyMat = cv::Mat::zeros(m_cDepthHeight, m_cDepthWidth, CV_8UC1);
...
void CHandTrace::ConvertImagePointToHand(const V3f& p, sAction1& act, const std::vector<sAction1> preHands)
{
	int index = 0;
		...
		if (Mark == 0) {
			if (p.x > 0 && p.x < m_cDepthWidth && p.y > 0 && p.y < m_cDepthHeight) {
				if (cv16Img.at<USHORT>(p.y, p.x) > 0) {
					//if (getValidInnerPoint(cv16Img, srcContours, p)) {
...
// 		for (auto it = m_vPixelPointRef.begin(); it != m_vPixelPointRef.end(); ++it) {
// 			int j = 0;
// 			for (; j < vDelID.size(); j++) {
// 				if ((*it)[0].x == vDelID[j])
// 					break;
