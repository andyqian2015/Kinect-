...
struct COORD3
{
	COORD3(int x0 = 0, int y0 = 0,int z0 = 0) :x(x0), y(y0), z(z0) {}
	float x;
	float y;
	float z;
};
...
struct sGesture
{
	sGesture() :gtype(GNone), t(0), fScale(1), fScreenRotateAngle(0){}
	...
	COORD3			    fScreenPivot;       //旋转中心  2个手的中心
};
...
{
	SGestureRegInitParam() :h(500), h1(50), Radius(25) {}
	int h;                    //判断可识别区域离屏距离//
	int h1;                   //将离屏的深度置0的深度//
...
typedef boost::function<void(std::vector<sGesture>)> funcallback;
__interface __declspec(uuid("{F053564F-ABCD-4208-91F6-2FABD73FD840}")) IEvent : public IDLPUnknown {
	...
	BOOL SetEnableGesture(eGestureType gtype, bool bEnable); //设置允许的手势//
	void GetGesture();                               //获取手势//

	void RegisterNofify(funcallback notifyFunc);
...
__interface __declspec(uuid("{EED3FE89-2B27-48E9-B0F0-8F31963ADA18}")) IKinectAction :public IDLPUnknown
{
	...
	void SetCalibImgPixelPoints(std::vector<nvmath::Vec2f> &CalibPixelPoints);  //通过点击图像点，获取到标定点的彩色图像像素点//
	...
	BOOL Calib(const std::vector<nvmath::Vec3f> & vMarkScreemPoints);
...
{
	BOOL InitData(const int &ImgWidth, const int &ImgHeight, const std::vector<nvmath::Vec3f> & vmarkscreenposIn, const std::vector<nvmath::Vec3f> & vmarkKinectPosIn, eInstallMode installMode, float h, float h1, float Radius, float ContourAreaThreshold);
...
//	void GetRegZone(std::vector<nvmath::Vec3f> & vPixelPos);
	...
	//test for use//
	void GetAllCounter(const int &colorwidth, const int & colorheight, const void* pColorBuffer, std::vector<nvmath::Vec2f>  &pBasePoint);
	//test for use//
...
//	void GetRegZone(std::vector<cv::Point> contours, std::vector<nvmath::Vec2f> & vPixelPos);
	...
	BOOL GetProContourDepthPoints(const void* pDepthBuffer, const int &nDepthWidth, const int &nDeptHeight, nvmath::Vec3f & vHandCenterPos);  //获取到投影到屏幕上的轮廓深度尖端点//
};
...
// struct rect {
// 
// 	Vec3<float> v_left_up;
// 	Vec3<float> v_left_down;
// 	Vec3<float> v_right_up;
// 	Vec3<float> v_right_down;
// };


// //标定
// __interface IGesutreCalibration{
// 
// 	void		Calibration(rect KinectRect,rect ScreemRect,Matrix44<float>  &TransformMat);
// 	void		GetScreemPos(const Vec3<float> & kinectPos, Vec2<float> &screempos);
// 
// 
// };
// typedef std::shared_ptr<IGesutreCalibration> PIGesutreCalibration;
...
__interface IDepthSensor
{
	...
	void		SetCoordinateTrans(int nPoints,COORD2* scr,COORD3* real);
};
...
__interface IGestureEngine
{
	...
	//大屏标定//
	//通过界面输入自动生成几个标记圆，则标记圆在屏幕中的位置可以知道//
	//通过彩色图像获取到每个像素点的ColorRGBX,然后利用图像识别的方法获取到对应的标定点的在彩色图像上的位置，然后转化到深度图像上的位置，得到对应的深度值//
...
//获取简单手势的接口//
__interface IGetHandTrace {
	bool Open();
	bool Close();
	void InitParam(); //标定和识别手势需要的数据结构//
};
...
{
	//V2
	//初始化KinectAction，用于获取KinectActionEvent数据//
