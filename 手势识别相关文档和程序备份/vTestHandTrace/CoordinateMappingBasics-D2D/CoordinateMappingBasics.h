...
class CCoordinateMappingBasics : public ICalibPro::ICalib
{
...
public:
	...
	int                                  pixelx;   
	int                                  pixely;       //代表x,y方向上的像素值//
	int                                  markdepth;    //标定点的深度值//

	V3f                                  getDepthData();
	std::vector<V3f>                     getMarkKinectPos();

	std::vector<V3f>                     vMarkKinectPos;

	//for test//
	std::vector<cv::Point2i>             vPixel;
	std::vector<cv::Point2i>             getPixel();


	int                                  testPixelx;
	int                                  testPixely;

	void                                setTestPixel(int testPixelx, int testPixely);
