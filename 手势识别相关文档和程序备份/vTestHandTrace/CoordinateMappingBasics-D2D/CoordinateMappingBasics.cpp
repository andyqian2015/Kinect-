...
/// <returns>status</returns>
 ...
 namespace ICalibPro {
	 PICalib CreatePICalib(){
		 PICalib p(new CCoordinateMappingBasics);
		 return p;
	 }
 }
...

{
        ...
        )
    {
        HRESULT hr = m_pCoordinateMapper->MapColorFrameToDepthSpace
			(nDepthWidth * nDepthHeight, (UINT16*)pDepthBuffer, nColorWidth * nColorHeight, m_pDepthCoordinates);
        if (SUCCEEDED(hr))
        {
            // loop over output pixels

			UINT pos = pixel_y * nColorWidth + pixel_x;

			RGBQUAD rgb = *(pColorBuffer + pos);

			int red = rgb.rgbRed;
			int blue = rgb.rgbBlue;
			int green = rgb.rgbGreen;
			
			int  normalRed, normalBlue, normalGreen;
			
			std::vector<V2f> depthPixelPoint;
			std::vector<V3f> depthData;
			...
			for (int i = 0; i < nColorHeight; i++) {
				for (int j = 0; j < nColorWidth; j++) {
					UINT testPos = i * nColorWidth + j;
					DepthSpacePoint testp = m_pDepthCoordinates[testPos];
					if (testp.X != -std::numeric_limits<float>::infinity() && testp.Y != -std::numeric_limits<float>::infinity()) {
						int testdepthX = static_cast<int>(testp.X + 0.5);
						int testdepthY = static_cast<int>(testp.Y + 0.5);

						if (abs(testdepthX - testPixelx) < 1 && abs(testdepthY - 95) < 1) {
							RGBQUAD  rgb = *(pColorBuffer + testPos) ;
							(m_pColorRGBX + testPos)->rgbRed = 255;
							(m_pColorRGBX + testPos)->rgbBlue = 0;
							(m_pColorRGBX + testPos)->rgbGreen = 0;
						}
					}
			...
			RGBQUAD normalRgb = *pColorBuffer;
			for (int i = 0; i < nColorWidth * nColorHeight; i++) {
				normalRgb = *(pColorBuffer + i);
				
				normalRed = normalRgb.rgbRed;
				normalBlue = normalRgb.rgbBlue;
				normalGreen = normalRgb.rgbGreen;
				if (abs(normalRed - 147) < 5 && abs(normalBlue - 71) < 5 && abs(normalGreen - 81) < 5)
				{
					int pixel_x = i % nColorWidth ;
					int pixel_y = i / nColorWidth ;
					V2f colorPixel(pixel_x, pixel_y);
					depthPixelPoint.push_back(colorPixel);
				}
			...
			V3f depthdata;
			DepthSpacePoint p = m_pDepthCoordinates[pos];
			...
			{
				depthX = static_cast<int>(p.X + 0.5);
				depthY = static_cast<int>(p.Y + 0.5);
				...
				UINT depthPos = depthY * nDepthWidth + depthX;
				depth = *(pDepthBuffer + depthPos);
				double fps = 0.0;
				...
				StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L" FPS = %0.2f    Time = %I64d      depth_Cursor = %d  depth_Cursor_max = %d  depth_Cursor_depth = %d    ",
					fps, (nTime - m_nStartTime), depthX, depthY, depth);
			...
			//test//
			//确定图像四个标记点的位置，暂时用于测试,用鼠标事件进行确定//

			
			if ((depthX != 0 || depthY != 0) && depth != 0 ) {
				V3f markpoint(depthX, depthY, depth);
				if (vMarkKinectPos.size() > 0) {
					float fpointdis = sqrt((depthX - vMarkKinectPos[vMarkKinectPos.size() - 1].x) *(depthX - vMarkKinectPos[vMarkKinectPos.size() - 1].x)
						+ (depthY - vMarkKinectPos[vMarkKinectPos.size() - 1].y) * (depthY - vMarkKinectPos[vMarkKinectPos.size() - 1].y));
					if (fpointdis >10)
						vMarkKinectPos.push_back(markpoint);
				}
				else {
					vMarkKinectPos.push_back(markpoint);
				}
			...
			if (vMarkKinectPos.size() == 4) {
				flag = true;
			}
				

			
			if (m_pKinectSensor && flag)
			{
				m_pKinectSensor->Close();
			}
			...
			m_pDrawCoordinateMapping->Draw(reinterpret_cast<BYTE*>(m_pColorRGBX), cColorWidth * cColorHeight * sizeof(RGBQUAD));
...
V3f  CCoordinateMappingBasics::getDepthData() {
	V3f depthData;
	depthData.x = pixelx;
	depthData.y = pixely;
	depthData.z = markdepth;
	return depthData;
}

std::vector<V3f>  CCoordinateMappingBasics::getMarkKinectPos() {
	return  vMarkKinectPos   ;
}


std::vector<cv::Point2i>  CCoordinateMappingBasics::getPixel()
{
	return vPixel;
}


void   CCoordinateMappingBasics::setTestPixel(int testPixelx, int testPixely) {
	this->testPixelx = testPixelx;
	this->testPixely = testPixely;
}
