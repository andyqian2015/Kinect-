...
{
	PIGetHandGesture createPIGetHandGesture(const std::vector<V3f> & vmarkscreenpos, const std::vector<V3f> & vmarkKinectPos, int KinectInstallPattern, float h, float h1, float Radius)
	{
		PIGetHandGesture p(new CDepthBasics(vmarkscreenpos, vmarkKinectPos,  KinectInstallPattern, h, h1, Radius));
		return p;
	}
}
...
void CDepthBasics::Registry(IGetHandGesturePro::CallBack callback)
{
	m_callback = callback;
}
...
/*				startTime = clock();*/
				...
				std::vector<sAction> handsgesture = m_PIHandTrace->getHands();

				m_callback(handsgesture);
