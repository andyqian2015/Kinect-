...
{
	CGSlideBackVert::CGSlideBackVert() :CGstEnsenssial()
	{
	...
	CGSlideBackVert::~CGSlideBackVert()
	{
		delete m_param;
	}

	void CGSlideBackVert::GstTell(const std::vector<sAction> &r, sGesture &g)
	{
		DataRecieve(r, g);
	...
	void CGSlideBackVert::BackConfirm(const SHANDGST &data, sGesture &g,int id)
	{
		...
		{
			bool backenough_x = (m_x_back > m_param->distance_back) || (m_x_back > (std::abs(m_x_max)*m_param->proportion_back));
			bool backenough_y = (m_y_back > m_param->distance_back) || (m_y_back > (std::abs(m_y_max)*m_param->proportion_back));
			if (backenough_x | backenough_y)
			{
				if (m_x_back < m_y_back)
				{
					if (std::abs(m_x_max) < m_param->distance_slide)
					{
	...
	void CGSlideBackVert::SetSlideBackParam(const sGestureParamSlideBack *gc)
	{
	...
	void CGSlideBackVert::ResetParam()
	{
	...
	sGestureParamSlideBack*	CGSlideBackVert::GetSlideBackParam()
	{
		return m_param;
	}

	void CGSlideBackVert::ResetCtn()
	{
		m_gstout.gtype = GNone;
// 		m_gstout.pos.x = 0.0;
// 		m_gstout.pos.y = 0.0;
...
// 		m_gstout.fScreenPivot.y = 0.0;
	...
	void CGSlideBackVert::GestureOut(sGesture &g)
	{
		g = m_gstout;
	}
}