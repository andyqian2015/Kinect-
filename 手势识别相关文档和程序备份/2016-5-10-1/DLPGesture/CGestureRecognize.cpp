...
// 		m_Gestures[15].pEng = CreateCGRotate();
	...
	void CGestureRecognize::OnAction(const std::vector<sAction> &r, std::vector<sGesture> &g)
	{
		if (r.size() < 1)return;
		sGesture s;
		s.gtype = DualHand;
	...
	int CGestureRecognize::SetGestureParam(const sGestureParam *gc)
	{
		if (!gc)
		{
			return 0;
	...
	void CGestureRecognize::EnableGesture(eGestureType gtype, bool bEnable)
	{
		...
		bool have;
		have = (m_Gestures[GMoveTo].bEnable || m_Gestures[GLongPush].bEnable || m_Gestures[GClick].bEnable || 
			m_Gestures[GDoubleClick].bEnable || m_Gestures[GSwithVert].bEnable || m_Gestures[GSwithHori].bEnable || 
			m_Gestures[GDrag].bEnable || m_Gestures[GZoom].bEnable || m_Gestures[GRotate].bEnable);
		if (have)
		{
	...
	// int  CGstRcn::SetGestureParam(const GESTURE_CONFIG& gb)
	// {
	// 	switch (gb.gesture)
	// 	{
	// 	case eG_Click:
