...
{
	// 转为二值图
	cvThreshold(m_g_pGrayImage, m_g_pBinaryImage, pos, 255, CV_THRESH_BINARY);
	// 显示二值图

	cv::Mat MbinaryMat(m_g_pBinaryImage, 0);

	findContours(MbinaryMat, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
 
	if (m_g_pBinaryImage) {
		m_g_pBinaryImage = nullptr;
		delete m_g_pBinaryImage;
	}
		

	if (m_g_pGrayImage)
	{
		m_g_pGrayImage = nullptr;
		delete m_g_pGrayImage;
	}	
}
...
{
	//利用二值图获取轮廓//
	m_g_pGrayImage = &IplImage(m_cv8Img);

	//将灰度图转化成二值图//
	m_g_pBinaryImage = cvCreateImage(cvGetSize(m_g_pGrayImage), IPL_DEPTH_8U, 1);
