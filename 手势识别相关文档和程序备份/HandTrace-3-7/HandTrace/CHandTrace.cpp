...
void CHandTrace::on_trackbar(int pos, std::vector<vector<Point>> & contours, cv::Mat & cv16Img)
{
	// 转为二值图
...
//对于获取的轮廓只有一个的情形，直接使用//
cv::Point2f CHandTrace::getSummitPoint(const std::vector<cv::Point> & srcContours, const cv::Mat & cv16Img, std::vector<cv::Point> & contours) {
	int k = 2;
	float threshold = 10;
	...
	if (contours.size() == 1) {
		...
		//对于手势尖端获取的这种方法并不谨慎，可能由于深度误差，使得尖端存在两个点是临近的，远离尖端的点只有一个点,对于手势尖端获取的这种方法并不谨慎，可能由于深度误差，使得尖端//
		//存在两个点是临近的，远离尖端的点只有一个点，所以区分出两个与屏幕有关的距离有关的包后，利用像素y值进行判断//
		//方法1:利用距离屏幕的距离的远近来判断尖端点，方法2：利用安装时的y值来判断距离屏幕的远近//
		////如果该包中只有两个点，则取其平均值
		vGroups = getGroup(k, contours, threshold);

		if (contours.size() == 3) {
			if (vGroups[0].size() == 1) {
				if (GetSummitPoint(vGroups[0][0].y, GetAvg(vGroups[1]).y))
				return vGroups[0][0];
				else {
					if (GetSummitPoint(vGroups[1][0].y, vGroups[1][1].y))
						return vGroups[1][0];
					else 
						return vGroups[1][1];
				}
				...
				if (GetSummitPoint(vGroups[1][0].y, GetAvg(vGroups[0]).y))
					return vGroups[1][0];
				else {
					if (GetSummitPoint(vGroups[0][0].y, vGroups[0][1].y))
						return vGroups[0][0];
					else
						return vGroups[0][1];
				}
			...
			cv::Point2f referpMin = getOptimalPoint(vGroups[iMinIndex]);
			cv::Point2f referpMax = getOptimalPoint(vGroups[iMaxIndex]);
			float fDist = 0;
			float fMaxDist = 0;
			float fMinDist = 10000;
			int iMaxDistIndex = 0;
			...
			if (KinectInstallPattern == 0) {
				...
				if (vGroups[iMaxIndex].size() == 1) {
					...
					for (int i = 0; i < vGroups[iMaxIndex].size(); i++) {
						cv::Point2f p(referpMin.x - vGroups[iMaxIndex][i].x, referpMin.y - vGroups[iMaxIndex][i].y);
				...
				//为了增加算法的普适性,考虑以后k>2的情形，特别的当最小包的点数目为1时候特别的拎出来讨论一下//
				if (vGroups[iMinIndex].size() == 1) {
					return vGroups[iMinIndex][0];
				}

				else {
					for (int i = 0; i < vGroups[iMinIndex].size(); i++) {
						cv::Point2f p(referpMin.x - vGroups[iMinIndex][i].x, referpMin.y - vGroups[iMinIndex][i].y);
						fDist = sqrt(p.x * p.x + p.y *p.y);
						if (fMinDist > fDist) {
							fMinDist = fDist;
							iMinDistIndex = i;
				...
				return vGroups[iMinIndex][iMinDistIndex];
			}
...
cv::Point2f CHandTrace::GetAvg(const std::vector<cv::Point> &vGroups)
{
	cv::Point2f vAvgPoint(0,0);
	for (int i = 0; i < vGroups.size(); i++)
	{
		vAvgPoint.x += vGroups[i].x;
		vAvgPoint.y += vGroups[i].y;
	}

	vAvgPoint.x = vAvgPoint.x / vGroups.size();
	vAvgPoint.y = vAvgPoint.y / vGroups.size();
	return vAvgPoint;
}
