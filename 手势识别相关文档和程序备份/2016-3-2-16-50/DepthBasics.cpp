...
void CDepthBasics::InitData(const std::vector<V3f> & vmarkscreenpos, const std::vector<V3f> & vmarkKinectPos, float h, float h1, float Radius) {
	...
	fActorOnOneSidethreshold = 15;  //主持人是否在一侧的阈值//
...
//			GetHandPos(cv16img, MyContours, imgpt);    //delete for contemporary retrieve the summitPoint of hand//
			...
			bool bOneSideflag = getSummitPoint(fActorOnOneSidethreshold, MyContours, fSummitPoint);  //主持人站在屏幕的一边//  
			...
			if (!bOneSideflag) {
				...
				if (bInScreenFlag && (MyContours.size()>0)) {
					...
					fSummitPoint = getSummitPoint(contours[i], cv16img, MyContours);    //用于获取尖端的像素//   //用于获取尖端的像素//  //********************// //delete for contemporary//
					//用于获取到指尖的位置//
				}
				//利用K-Means方法获取到手势尖端点//
				//test//
	...
	if (m_hands.size() >0) {
		...
		for (int i = 0; i < m_hands.size(); i++) {
			out1 << m_hands[i].ID << " " << m_hands[i].pos.x << " " << m_hands[i].pos.y << " " << m_hands[i].pos.z << " " << m_hands[i].st << std::endl;
		}
