...
#define HAND_LIKELY_AREA 40
	...
	for (unsigned i = 0; i < contours.size(); i++)
	{
		...
		{   //比较有可能像手的区域
			...
			//test//
			if (fSummitPoint.x == 0 || fSummitPoint.y == 0) {
				ATLTRACE("%d\n", 22);
			}
			...
			vSummitPoint.push_back(fSummitPoint);  //用于确定是否是同一个手势//


			//test//
			m_HandNum = vfKinectPos.size();  //test:为检测出多少个手服务//
		}
	}
...
cv::Point2f CHandTrace::getSummitPoint(std::vector<cv::Point> & contours) {
	...
	if (contours.size() == 1) {
		...
		int minDist = 10000;
		for (int i = 0; i < contours.size(); i++) {
			V3f pixelPoint(contours[i].x, contours[i].y, cv16Img.at<USHORT>(contours[i].y, contours[i].x));
			V3f KinectPos = getKinectPos(pixelPoint);
			V3f ImgProjectPos = getProjectPos(KinectPos);
...
std::vector<sAction> CHandTrace::getHands()
{
	return m_hands;
}
