...
//			cen = pMyPoint;
	...
	if (vPos.size() > 0) {
		...
		//处理方法：手势丢失或者是手势离开的同时有新手势进来：利用阈值进行判定，如果上一帧判定的手势的在屏幕坐标和当前帧的坐标距离差超过阈值的2倍，则认为不是同一个手势，否则认为vCopyPreHands[k]为丢失掉的手势，m_Hands为新增的手势//
		...
		//为了不至于引起混乱，可以先将当前帧的pos调整顺序，最后同意赋予vPos//
...
void CDepthBasics::ConvertImagePointToHand(const V3f& img, sAction& act)
{
	...
	//如果前一刻有状态，为down或者是Move,而此刻没有状态，那么此刻的状态定义为up，为防止未检测出来手势的轮廓从而带来的误操作，//
	//对手势的尖端设定一个阈值，如果手势是在识别的框架边缘，下一刻识别不到手势，即认定当前的手势为none，否则即认定为未识别出手势的位置按照前面两帧的方向和速度拟合当前帧的手势状态//

	//暂时不考虑误操作的情形，给定一个阈值，先测试一下//

	
	//设定在识别区域中的点到四个边界平面的距离，去取最小的作为判断下一刻，该点是否已经出了区域//
	const int iUpThreshold = 10;
